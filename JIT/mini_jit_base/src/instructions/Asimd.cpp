...
uint32_t mini_jit::instructions::Asimd::lsStrImmUnsOff(uint8_t i_regSimdDes,
                                                       ...
                                                       uint16_t i_imm12) {
  uint32_t l_ins =
      lsLdrImmUnsOff(i_regSimdDes, i_regGpAddr, i_regSize, i_imm12);
  l_ins &= ~(0b1 << 22);

  return l_ins;
}

uint32_t mini_jit::instructions::Asimd::lsLd1MultipleNoOff(
    uint8_t i_regSimdDes, uint8_t i_regGpAddr, arrspec_t i_arrSpec,
...
uint32_t mini_jit::instructions::Asimd::lsSt1MultipleNoOff(
    uint8_t i_regSimdDes, uint8_t i_regGpAddr, arrspec_t i_arrSpec,
    uint8_t i_nStructures) {
  uint32_t l_ins =
      lsLd1MultipleNoOff(i_regSimdDes, i_regGpAddr, i_arrSpec, i_nStructures);
  l_ins &= ~(0b1 << 22);
