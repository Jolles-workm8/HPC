...
uint32_t mini_jit::instructions::Asimd::dpFmadd( uint8_t   i_regSimdDes,
  ...
  l_ins |= 0x1f & i_regSimdDes;
  l_ins |= (0x1f & i_regSimdSrc0) << 5;
  l_ins |= (0x1f & i_regSimdSrc1) << 10;
...
uint32_t mini_jit::instructions::Asimd::dpFmlaVector( uint8_t   i_regSimdDes,
                                                      uint8_t   i_regSimdSrc0,
                                                      uint8_t   i_regSimdSrc1,
                                                      arrspec_t i_arrSpec ) {
  uint32_t l_ins = 0xe20CC00;

  l_ins |= 0x1f & i_regSimdDes;
  l_ins |= (0x1f & i_regSimdSrc0) << 5;
  l_ins |= (0x1f & i_regSimdSrc1) << 16;
...
uint32_t mini_jit::instructions::Asimd::dpFmlaElement( uint8_t i_regSimdDes,
                                                       uint8_t i_regSimdSrc0,
                                                       uint8_t i_regSimdSrc1,
                                                       uint8_t i_arrSpec,
                                                       uint8_t i_index ) {
  ...
  else if( i_arrSpec == arrspec_t::d1 ) {
    l_ins |= 0x1 << 22;
    l_ins |= 0b1 << 28;
    l_ins |= 0b1 << 30;
  }
  ...
  // set index
  if( i_arrSpec == arrspec_t::s4 || i_arrSpec == arrspec_t::s2 || i_arrSpec == arrspec_t::s1 ) {
    l_ins |= (0b1 & i_index) << 21;
    l_ins |= (0b10 & i_index) << 10; // +1 already in mask
  }
  if( i_arrSpec == arrspec_t::d2 || i_arrSpec == arrspec_t::d1 ) {
    l_ins |= 0b1 << 11;
  }
